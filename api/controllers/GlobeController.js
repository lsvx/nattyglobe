...
module.exports = {
	...
	ready: function(req, res) {
		...
		Location.find().done(function(err, locations) {
			var locationsAr = [];
			locationsAr = locations.map(function(location) {
				var timestamps = [];
				timestamps = location.timestamps.filter(function(locindex, timestamp) {
					return (curr - timestamp > 300000);
				});
				location.timestamps = timestamps;
				return location;
			});
			return res.json(locationsAr);
		});
	},
